...
from .cnn_model import resnet_act_simple as cnn_simple_model
...
from .set_utils import (
    build_dense_dropout_model_2, PaddedToSegments, SegmentAggregation,
...
class RESNET_forward_act(tf.keras.Model):
	dense_options = {
        'activation': 'relu',
	...
	def __init__(self, output_activation, output_dims, n_dense_layers, dense_width, dense_dropout, max_timescale,n_positional_dims, phi_width):
		...
		super().__init__()
		self.cnn_model = cnn_simple_model()
	...
	def build(self, input_shapes):
		print(input_shapes)
		n_feature_maps = 64
		demo, times, values, measurements, locs, lengths= input_shapes
		self.resnet_model = self.cnn_model.build_resnet(values[1:])
		cnn_output_shape = (None, None,128)
	...
	def call(self, inputs):
		...
		# pdb.set_trace()
		# cnn_ip = tf.concat([self.values, self.measurements], -1)
