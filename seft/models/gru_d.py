...
class GRUDCell(GRUCell):
    ...
    def build(self, input_shape):
        ...
        # asd = self.kernel.shape[-1]
        # self.kernel_z = self.kernel[:,0:int(asd/3)]
        # self.kernel_r = self.kernel[:,int(asd/3):2*(int(asd/3))]
        # self.kernel_h = self.kernel[:,2*(int(asd/3)):]
    ...
    def call(self, inputs, states, training=None):
        ...
        (self.masking_kernel_z,
         ...
         self.masking_kernel_h) = tf.split(self.masking_kernel, 3, axis=-1)
