...
from tensorflow.keras.layers import Dense, Dropout, BatchNormalization, LayerNormalization, Conv1D, Activation, LeakyReLU
import pdb
...
class PaddedToSegments(tf.keras.layers.Layer):
    ...
    def call(self, inputs, mask):
        ...
        # tf.print('pad_segments', tf.shape(mask), tf.shape(inputs), tf.shape(valid_observations))
...
class SegmentAggregation(tf.keras.layers.Layer):
    ...
    def call(self, data, segment_ids):
        ...
        sort_order = tf.argsort(segment_ids)
        # tf.print(sort_order)
        return self.aggregation_fn(tf.gather(data, sort_order), tf.gather(segment_ids, sort_order))
...
def build_dense_dropout_model_2(n_layers, width, dropout, dense_kwargs,
                              name=None):
    ...
    if dropout > 0.:
        layers = list(chain(*(
            (Conv1D(filters=width, kernel_size=1), Activation('relu'))
            for i in range(n_layers)
        )))
    else:
        layers = [Dense(width, **dense_kwargs) for i in range(n_layers)]
        # [Dense(width, **dense_kwargs) for i in range(n_layers)]
