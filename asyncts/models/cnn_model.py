...
import pdb
class attention():
    def __init__(self, dot_prod_dim, n_heads):
        super().__init__()
        self.n_heads = n_heads
        self.key_dim = dot_prod_dim
    def build_att_model(self,input1, input2, mask):
        # input1, input2 = input_shape
        # pdb.set_trace()
        input_layer1 = keras.Input(input1)
        input_layer2 = keras.Input(input2)
        mask_layer = keras.Input(mask)
        # pdb.set_trace()
        att_x = keras.layers.MultiHeadAttention(num_heads=self.n_heads, key_dim=self.key_dim)(input_layer1, input_layer2, attention_mask = mask_layer)



        # att_x = keras.layers.MultiHeadAttention(num_heads=self.n_heads, key_dim=self.key_dim, attention_axes=-2, dropout=0.2)(att_x, att_x)

        # pdb.set_trace()
        self.att = tf.reduce_sum(att_x, -2)
        # self.att = keras.layers.GlobalAveragePooling1D()(att_x)
        # self.att = keras.layers.BatchNormalization()(self.att)
        model = keras.Model((input_layer1, input_layer2, mask_layer), self.att)
        return model
    # def call(self, inputs):
    #     input1, input2 = inputs
    #     return self.att(input1,input2)
...
# class sin_activation(keras.layers.Layer):
#     def __init__(self):
#         super().__init__()
#     def build(self):
#         sin_act = tf.math.sin
#     def call(self, inputs):
#         return sin_act(inputs)
...
class cnn_model():
    ...
    def build_resnet(self, input_shapes):
        ...
        conv_x = keras.layers.Conv1D(filters=n_feature_maps, kernel_size=1,  padding='same')(input_layer1)
        # conv_x = keras.layers.BatchNormalization()(conv_x)
        conv_x = keras.layers.Activation('relu')(conv_x)
        # conv_x = conv_x*tf.tile(input_correction, [1,1,n_feature_maps])

        conv_y = keras.layers.Conv1D(filters=n_feature_maps, kernel_size=1,  padding='same')(conv_x)
        
        # conv_y = keras.layers.BatchNormalization()(conv_y)
        conv_y = keras.layers.Activation('relu')(conv_y)
        # conv_y = conv_y*tf.tile(input_correction, [1,1,n_feature_maps])

        conv_z = keras.layers.Conv1D(filters=n_feature_maps, kernel_size=1,  padding='same')(conv_y)
        ...
        conv_y = keras.layers.Conv1D(filters=n_feature_maps*2, kernel_size=1,  padding='same')(conv_x)
        # 
        # conv_y = keras.layers.BatchNormalization()(conv_y)
        conv_y = keras.layers.Activation('relu')(conv_y)
        ...
        # self.gap_layer = (gap_layer_1 + gap_layer_2)/2
        # self.gap_layer = gap_layer_1
        # self.gap_layer = keras.layers.GlobalAveragePooling1D()(output_block_4)
        # self.gap_layer = tf.expand_dims(self.gap_layer,0)
        # self.gap_layer = keras.layers.GlobalAveragePooling1D()(output_block_3)
        # self.gap_layer = tf.cumsum(output_block_3,axis=1)
        # weights = tf.cumsum(tf.ones_like(self.gap_layer),axis=1)
        # self.gap_layer = self.gap_layer/weights
