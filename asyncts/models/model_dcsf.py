...
class DCSF(tf.keras.Model):
	...
	def data_preprocessing_fn(self):
		...
		def flatten_unaligned_measurements(ts, labels):
			...
			X = tf.tile(X[:,None], (1, q))
			
			tims_max = tf.tile(tf.math.reduce_max(X, 0)[None,:], (p,1))
			tims = tf.math.divide_no_nan(X, tims_max)
			...
			tims_pad = tf.concat([tims, tf.cast(0*padds, dtype=tf.float32)], 0)
			...
			gathered_tims = tf.gather_nd(tf.transpose(tims_pad), mask_positions)

			values = tf.concat([gathered_channs, gathered_tims[:,None], gathered_vals[:,None]], -1)
...
# demo_outs
			return (demo, gathered_tims, values, gathered_avgps, gathered_inds), labels
